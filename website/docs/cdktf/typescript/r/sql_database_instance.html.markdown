...
class MyConvertedCode extends cdktf.TerraformStack {
  constructor(scope: constructs.Construct, name: string) {
    ...
    const googleComputeInstanceAppsCount = cdktf.TerraformCount.of(
      cdktf.Token.asNumber("8")
    );
    const googleComputeInstanceApps =
      new google.computeInstance.ComputeInstance(this, "apps", {
        bootDisk: {
          initializeParams: {
        ...
        name:
          ...
          cdktf.Token.asString(
            cdktf.Op.add(googleComputeInstanceAppsCount.index, 1)
          ),
        networkInterface: [
          {
            accessConfig: [{}],
        ...
        count: googleComputeInstanceAppsCount,
      });
    ...
    you should consider using a for loop. If you are looping over something only known to Terraform, e.g. a result of a data source
    you need to keep this like it is.*/
    const googleSqlDatabaseInstancePostgresDynamicIterator0 =
      cdktf.TerraformIterator.fromList(
        cdktf.Token.asAny(googleComputeInstanceApps)
      );
    new google.sqlDatabaseInstance.SqlDatabaseInstance(this, "postgres", {
      ...
      settings: {
        ipConfiguration: {
          authorizedNetworks:
            googleSqlDatabaseInstancePostgresDynamicIterator0.dynamic({
              name: appsValue.name,
              value: cdktf.propertyAccess(appsValue.networkInterface, [
                "0",
                "access_config",
                "0",
                "nat_ip",
...
The optional `settingsAdvancedMachineFeatures` subblock supports:

* `threadsPerCore` - (Optional) The number of threads per core. The value of this flag can be 1 or 2. To disable SMT, set this flag to 1. Only available in Cloud SQL for SQL Server instances. See [smt](https://cloud.google.com/sql/docs/sqlserver/create-instance#smt-create-instance) for more details.
...
* `retainedBackups` - (Optional) Depending on the value of retention_unit, this is used to determine if a backup needs to be deleted. If retention_unit
...
<!-- cache-key: cdktf-0.17.0-pre.15 input-56a88b0185aff4fc6a97bdc64e7c25019e17a4bec07d74d49b67d7c2c5c35bef -->