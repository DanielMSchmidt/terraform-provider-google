      ...
      rule: [
        {
          ...
          priority: cdktf.Token.asNumber("1000"),
        },
        {
          action: "allow",
          ...
          priority: cdktf.Token.asNumber("2147483647"),
        },
      ],
    });
  }
...
* `banDurationSec` - (Optional) Can only be specified if the `action` for the rule is "rate_based_ban".
    If specified, determines the time (in seconds) the traffic will continue to be banned by the rate limit after the rate falls below the threshold.

* `banThreshold` - (Optional) Can only be specified if the `action` for the rule is "rate_based_ban".
...
* `enforceOnKeyConfigs` - (Optional, [Beta](https://terraform.io/docs/providers/google/guides/provider_versions.html)) If specified, any combination of values of enforce_on_key_type/enforce_on_key_name is treated as the key on which ratelimit threshold/action is enforced. You can specify up to 3 enforce_on_key_configs. If `enforceOnKeyConfigs` is specified, enforce_on_key must be set to an empty string. Structure is [documented below](#nested_enforce_on_key_configs).
...
<!-- cache-key: cdktf-0.17.0-pre.15 input-57a49eddeb2afb89e6ac690fc2a5a71c5aa163855da0545cc95a07483a087ce8 -->