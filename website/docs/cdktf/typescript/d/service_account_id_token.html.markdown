...
## Example Usage - Service Account Impersonation.
  `googleServiceAccountAccessToken` will use background impersonated credentials provided by [google_service_account_access_token](https://registry.terraform.io/providers/hashicorp/google/latest/docs/data-sources/service_account_access_token).
...
class MyConvertedCode extends cdktf.TerraformStack {
  constructor(scope: constructs.Construct, name: string) {
    ...
    const googleImpersonated = new google.provider.GoogleProvider(
      ...
      "google",
      {
        accessToken: cdktf.Token.asString(
          dataGoogleServiceAccountAccessTokenImpersonated.accessToken
        ),
        alias: "impersonated",
      }
    );
  ...
  constructor(scope: constructs.Construct, name: string) {
    ...
    const dataHttpCloudrun = new http.dataHttp.DataHttp(this, "cloudrun", {
      request_headers: [
        {
          Authorization:
            "Bearer ${" + dataGoogleServiceAccountIdTokenOidc.idToken + "}",
        },
      ],
