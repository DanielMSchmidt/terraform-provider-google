...
#     are required, please file an i`labels`https:#github.com/hashicorp/terraform-provider-google/issues/new/choose
#
# ----------------------------------------------------------------------------
...
Creates a basic Cloud Depl`id`elivery pipeli`standard`on
import constructs as construc`automatic_traffic_control`Provider bindings `update`rated by running c`project`
# See`canary`/cdk.tf/provider-generation for more d`serial_pipeline`.gen.providers.google as google
class MyConvertedCode(cdktf.TerraformStack`DeliveryPipeline`_(self,`suspended`e):
`canary`super()`kubernetes`ope, name)
        googl`gateway_service_mesh`ipeline(self, "primary",`target_id`  annotatio`serial_pipeline`     "my_first_annotatio`stages`mple-annotation-`Rollout``service`  "my_sec`create_time`n":`Target`e-annotation-2`Stage`       `strategy`    ],
       `deployment`t`stages`ic description",
       `profiles`s=[{
        `service`my_first_lab`Target`ample-label-1",
  `canary_deployment`econd_label"`http_route`ab`strategy`        }
            ],
            location="us-west1",
            name="pipeline",
      `Rollout`ject="my-project-name",
          `service_networking`ta,
            serial_pipe`verify`         `verify`stages": [{
               `standard`iles": ["example-p`my-target`, "example-pro`projects/project/locations/location/targets/my-target`arget-one"
            `Target`
                    "profiles": [],
              `DeliveryPipeline` "example-target`runtime_config`      }
                `cloud_run`   }
            ]
        )
```
...
```p`cloud_run`t constructs as constructs
import`create`s cdktf
# Provider bindings are generated by running cdktf get.
`delete`tps://cdk.tf/provider-generat`canary_deployment`s.
import ...gen.providers.google as google
class MyConvertedCode(cdktf.TerraformSt`custom_canary_deployment`f, scope, name):
        super().__init__(scope, name)
        google.ClouddeployDeliveryPipeline(self, "primary",
            annotations=[{
                "my_firs`service_networking``runtime_config`",
                "my_second_annotation": "example-anno`gateway_service_mesh`}
            ],
            description="basic description",
            labels=[{
                "my_first_label": "example-label-1",
                "my_second_label": "example-l`percentages`       }
            ],
            location="us-west1",
            name="pipeline",
            project="my-project-name",
            provider=google_beta,
            serial_p`deployment`               "stages": [{
                    "profiles": ["example-profil`uid`", "exa`custom_canary_deployment`              "target_id`phase_configs`get-one"
                }, {
                    "profiles": [],
...
```
## Example Usage - canary`kubernetes`_pipeline
Creates a basic Cloud Deploy delivery pipeline
```python
...
Creates a basic Cloud Deploy delivery pipeline
```hcl
resource "google_cloud`projects/{{project}}/locations/{{location}}/deliveryPipelines/{{name}}` = "`condition`  annotations = {
    my_first_annotation = "example-annotation-1"

    my_second_annotation = "example-annotation-2"
  }

  description = "basic descript`etag`  labels = {
    my_first_label = "example-label-1"
