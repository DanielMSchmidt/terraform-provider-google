...
in a `2X` series release on `terraform init`.
...
of by the `enableFlowLogs` variable. Users with `enable_flow_logs = false` only need to remove the field.
...
1. Add a `googleComputeSubnetwork` to your config, import it using `terraform import`
1. Reference the subnetwork using the `subnetwork` field on your `googleContainerCluster`
...
class MyConvertedCode extends cdktf.TerraformStack {
  constructor(scope: constructs.Construct, name: string) {
      ...
      new google.computeSubnetwork.ComputeSubnetwork(
        ...
        "container_subnetwork",
        {
          ...
          network: cdktf.Token.asString(
            googleComputeNetworkContainerNetwork.selfLink
          ),
          region: "us-central1",
        }
      );
    new google.containerCluster.ContainerCluster(this, "primary", {
      ...
      location: "us-central1",
      name: "my-cluster",
      network: cdktf.Token.asString(googleComputeNetworkContainerNetwork.name),
...
### At least one of `allow_null_header ` or `segmentTerminator` is now required on `googleHealthcareHl7V2StoreParserConfig`
...
class MyConvertedCode extends cdktf.TerraformStack {
  constructor(scope: constructs.Construct, name: string) {
    ...
    const googleProjectServiceServiceForEachIterator =
    ...
    new google.projectService.ProjectService(this, "service", {
      disableOnDestroy: false,
      project: "your-project-id",
      service: cdktf.Token.asString(
        googleProjectServiceServiceForEachIterator.key
      ),
      forEach: googleProjectServiceServiceForEachIterator,
    });
  }
}
