...
class MyConvertedCode extends cdktf.TerraformStack {
  constructor(scope: constructs.Construct, name: string) {
    ...
    const googleComputeNodeTemplateSoletenantTmpl =
      new google.computeNodeTemplate.ComputeNodeTemplate(
        this,
        "soletenant-tmpl",
        {
    ...
    new google.computeNodeGroup.ComputeNodeGroup(this, "nodes", {
      ...
      nodeTemplate: cdktf.Token.asString(
        googleComputeNodeTemplateSoletenantTmpl.id
      ),
      size: 1,
      zone: "us-central1-f",
    });
...
* `mode` -
  ...
  Possible values are: `off`, `on`, `onlyScaleOut`.
...
* `shareType` -
  ...
  Possible values are: `organization`, `specificProjects`, `local`.
