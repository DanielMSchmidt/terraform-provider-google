...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        google_compute_network_vpc = google.compute_network.ComputeNetwork(self, "vpc",
            name="conn-test-net"
        )
        data_google_compute_image_debian9 =
        google.data_google_compute_image.DataGoogleComputeImage(self, "debian_9",
            family="debian-11",
            project="debian-cloud"
        )
        google_compute_instance_destination =
        google.compute_instance.ComputeInstance(self, "destination",
            boot_disk=ComputeInstanceBootDisk(
                initialize_params=ComputeInstanceBootDiskInitializeParams(
                    image=cdktf.Token.as_string(data_google_compute_image_debian9.id)
                )
            ),
            ...
            network_interface=[ComputeInstanceNetworkInterface(
                ...
                network=cdktf.Token.as_string(google_compute_network_vpc.id)
            )
            ]
        )
        google_compute_instance_source =
        google.compute_instance.ComputeInstance(self, "source",
            boot_disk=ComputeInstanceBootDisk(
                initialize_params=ComputeInstanceBootDiskInitializeParams(
                    image=cdktf.Token.as_string(data_google_compute_image_debian9.id)
                )
        ...
        google.network_management_connectivity_test.NetworkManagementConnectivityTest(self, "instance-test",
            destination=NetworkManagementConnectivityTestDestination(
                instance=cdktf.Token.as_string(google_compute_instance_destination.id)
            ),
            name="conn-test-instances",
            protocol="TCP",
            source=NetworkManagementConnectivityTestSource(
                instance=cdktf.Token.as_string(google_compute_instance_source.id)
            )
        )
```
<div class = "oics-button" style="float: right; margin: 0 0 -15px">
...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        google_compute_subnetwork_subnet =
        google.compute_subnetwork.ComputeSubnetwork(self, "subnet",
            ip_cidr_range="10.0.0.0/16",
            name="connectivity-vpc-subnet",
            network=cdktf.Token.as_string(google_compute_network_vpc.id),
            region="us-central1"
        )
        google_compute_address_dest_addr =
        google.compute_address.ComputeAddress(self, "dest-addr",
            address="10.0.43.43",
            address_type="INTERNAL",
            name="dest-addr",
            region="us-central1",
            subnetwork=cdktf.Token.as_string(google_compute_subnetwork_subnet.id)
        )
        google_compute_address_source_addr =
        google.compute_address.ComputeAddress(self, "source-addr",
            address="10.0.42.42",
            address_type="INTERNAL",
            name="src-addr",
            region="us-central1",
        ...
        google.network_management_connectivity_test.NetworkManagementConnectivityTest(self, "address-test",
            destination=NetworkManagementConnectivityTestDestination(
                ip_address=cdktf.Token.as_string(google_compute_address_dest_addr.address),
                network=cdktf.Token.as_string(google_compute_network_vpc.id),
                project_id=cdktf.Token.as_string(google_compute_address_dest_addr.project)
            ),
            name="conn-test-addr",
            protocol="UDP",
            source=NetworkManagementConnectivityTestSource(
                ip_address=cdktf.Token.as_string(google_compute_address_source_addr.address),
                network=cdktf.Token.as_string(google_compute_network_vpc.id),
                network_type="GCP_NETWORK",
                project_id=cdktf.Token.as_string(google_compute_address_source_addr.project)
            )
        )
```
...
<!-- cache-key: cdktf-0.17.0-pre.15 input-7a4e679a95c210677bb72fd5709ef2dc4b9ee6b8aa1c9d2e8f63387066c0e49b -->